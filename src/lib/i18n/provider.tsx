...
export function EnhancedTranslationProvider({
  ...
  const reportMissingKey = useCallback((key: string, namespace?: string) => {
    ...
    if (i18nConfig.fallback.logMissingKeys) {
      console.warn(`Missing translation key: "${key}" in locale: "${locale}"${namespace ? ` (namespace: ${namespace})` : ""}`);
    }
  }, [locale, onMissingKey]);

  const reportError = useCallback((error: Error, key: string) => {
  ...
  useEffect(() => {
    if (i18nConfig.validation.enabled && messages) {
      ...
      if (missingKeys.length > 0) {
        console.warn(`Found ${missingKeys.length} missing translation keys in locale "${locale}":`, missingKeys);
      }
    }
  }, [messages, locale]);
...
export function useEnhancedTranslationContext() {
  ...
  if (!context) {
    throw new Error("useEnhancedTranslationContext must be used within an EnhancedTranslationProvider");
  }
...
export function useTranslationDebug() {
  ...
  const logTranslationUsage = useCallback((key: string, namespace?: string, value?: string) => {
    if (!debugMode) return;
  ...
  const checkKeyExists = useCallback((key: string, namespace?: string) => {
    if (!debugMode) return true;
...
export function withTranslationDebug<P extends object>(
  ...
  const WrappedComponent = (props: P) => {
    ...
    useEffect(() => {
      if (namespace) {
        logTranslationUsage("component_mounted", namespace, Component.displayName || Component.name);
      }
    }, [logTranslationUsage]);
