...
export function useSafeTranslations(namespace?: string) {
  ...
  const safeT = useCallback((key: string, params?: Record<string, any>, fallback?: string) => {
    ...
    try {
      ...
      if (i18nConfig.fallback.logMissingKeys) {
        console.warn(`Missing translation key: "${key}" in namespace: "${namespace || 'default'}" for locale: "${locale}"`);
      }
  ...
  const getTranslationOrFallback = useCallback((key: string, fallback: string, params?: Record<string, any>) => {
    return safeT(key, params, fallback);
  }, [safeT]);

  const formatMessage = useCallback((key: string, params?: Record<string, any>) => {
...
export function useAccessibilityTranslations() {
  ...
  const getAriaLabel = useCallback((key: string, fallback?: string) => {
    ...
    if (!translation || translation === key || translation.startsWith('[')) {
      const defaultFallback = fallback || key.replace(/([A-Z])/g, ' $1').toLowerCase().trim();
  ...
  const getSkipLink = useCallback((target: string) => {
    return safeT(`skipTo${target.charAt(0).toUpperCase() + target.slice(1)}`, undefined, `Skip to ${target}`);
  }, [safeT]);

  const getNavigationLabel = useCallback((type: "main" | "mobile" | "footer") => {
...
export function useAdminTranslations() {
  ...
  const getOrderStatus = useCallback((status: "pending" | "confirmed" | "processing" | "shipped" | "delivered" | "cancelled") => {
    return safeT(status, undefined, status);
  }, [safeT]);

  const getProductStatus = useCallback((status: "active" | "inactive") => {
    return safeT(status, undefined, status);
  }, [safeT]);

  const getAlertLevel = useCallback((level: "waiting" | "warnings" | "unacknowledged") => {
...
export function useUITranslations() {
  ...
  const getButtonLabel = useCallback((action: string) => {
    const key = `${action}Button`;
    return safeT(key, undefined, action);
  }, [safeT]);

  const getMenuLabel = useCallback((type: "main" | "mobile" | "user") => {
    const key = `${type}Menu`;
    return safeT(key, undefined, `${type} menu`);
  }, [safeT]);

  const getLoadingText = useCallback((context?: string) => {
    const key = context ? `loading${context.charAt(0).toUpperCase() + context.slice(1)}` : "loading";
...
export function useTranslationValidation() {
  ...
  const validateTranslations = useCallback((requiredKeys: string[], context?: string) => {
  ...
  const reportMissingKey = useCallback((key: string, context?: string) => {
    if (!validationEnabled || !key) return;

    setMissingKeys(prev => {
      if (!prev.has(key)) {
        console.warn(`Missing translation key: "${key}" for locale: "${locale}"${context ? ` (context: ${context})` : ""}`);
...
export function useTranslationContext() {
  ...
  const pushNamespace = useCallback((namespace: string) => {
  ...
  const getNamespacedKey = useCallback((key: string, namespace?: string) => {
