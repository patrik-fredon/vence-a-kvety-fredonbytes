...
jest.mock("next-intl/server", () => ({
  getTranslations: jest.fn(() => (key: string) => {
    const translations: Record<string, string> = {
      ...
      "hero.description":
        "V těžkých chvílích rozloučení vám pomůžeme vyjádřit lásku a úctu prostřednictvím nádherných květinových aranžmá. Každý věnec vytváříme s citem, pečlivostí a porozuměním pro vaši ztrátu.",
      ...
      "benefits.items.0.description":
        "Květiny objednáváme vždy čerstvé až na základě vaší objednávky. Garantujeme nejvyšší kvalitu a dlouhou výdrž každého květu pro důstojné rozloučení.",
      "benefits.items.1.title": "Spolehlivé doručení na míru",
      "benefits.items.1.description":
        "Produkty doručíme přesně na vámi určené místo v čase, který vám vyhovuje. Rozumíme naléhavosti a zajistíme včasné dodání bez stresu.",
      "benefits.items.2.title": "Pečlivá ruční práce",
      "benefits.items.2.description":
        "Dáváme si záležet na uložení každého jednoho květu tak, aby výsledná práce byla dokonalá. Každý věnec je jedinečné dílo vytvořené s láskou a úctou.",
      "benefits.items.3.title": "Možnost personalizace",
      "benefits.items.3.description":
        "Máte možnost si vybrat tvar, velikost a barevnost vašeho smutečního věnce podle vlastních představ. Pomůžeme vám vytvořit osobní a významné rozloučení.",

      // Features section (existing)
      "features.handcrafted.title": "Ruční výroba",
      "features.handcrafted.description":
      ...
      "features.personalApproach.description":
...
describe("Homepage Content Rendering", () => {
  ...
  describe("Hero Section", () => {
    it("should display main title correctly", async () => {
      ...
      expect(screen.getByRole("heading", { level: 1 })).toHaveTextContent(
        "Důstojné rozloučení s krásou květin"
      );
    });
    ...
    it("should display subtitle correctly", async () => {
      ...
      expect(
        screen.getByText("Pohřební věnce vytvořené s láskou a úctou k vašim blízkým")
      ).toBeInTheDocument();
    });
    ...
    it("should display description correctly", async () => {
      ...
      expect(
        screen.getByText(/V těžkých chvílích rozloučení vám pomůžeme vyjádřit lásku a úctu/)
      ).toBeInTheDocument();
    });
  ...
  describe("Philosophy Section", () => {
    it("should display philosophy quote", async () => {
      ...
      expect(
        screen.getByText(/Život je křehký jako motýlí prach, ale krása a láska zůstávají navždy/)
      ).toBeInTheDocument();
    });
    ...
    it("should display philosophy text", async () => {
      ...
      expect(
        screen.getByText(/Uvědomujeme si, že život je křehký jako motýlí prach/)
      ).toBeInTheDocument();
    });
  ...
  describe("Benefits Section", () => {
    it("should display benefits title", async () => {
      ...
      expect(screen.getByRole("heading", { level: 2 })).toHaveTextContent(
        "Proč si vybrat naše pohřební věnce"
      );
    });
    ...
    it("should display benefit descriptions", async () => {
      ...
      expect(
        screen.getByText(/Květiny objednáváme vždy čerstvé až na základě vaší objednávky/)
      ).toBeInTheDocument();
      expect(screen.getByText(/Produkty doručíme přesně na vámi určené místo/)).toBeInTheDocument();
      expect(
        screen.getByText(/Dáváme si záležet na uložení každého jednoho květu/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Máte možnost si vybrat tvar, velikost a barevnost/)
      ).toBeInTheDocument();
    });
  ...
  describe("Responsive Design", () => {
    ...
    it("should have responsive button layout", async () => {
      ...
      const buttonContainer = screen.getByRole("link", {
        name: "Vybrat věnec pro rozloučení",
      }).parentElement;
