...
const TestWrapper = ({ children, locale = 'cs' }: { children: React.ReactNode; locale?: string }) => {
...
describe('Homepage Conversion Elements', () => {
  ...
  describe('Hero Section CTA Buttons', () => {
    ...
    test('should display hero title with dignified messaging', async () => {
      ...
      expect(heroTitle).toHaveClass('text-elegant', 'text-5xl', 'md:text-6xl', 'font-semibold', 'text-primary-800');
    });
  ...
  describe('Trust-Building Benefits Section', () => {
    test('should display benefits section title', async () => {
      ...
      const benefitsTitle = screen.getByRole('heading', { level: 2, name: /proč si vybrat naše pohřební věnce/i });
      expect(benefitsTitle).toBeInTheDocument();
      expect(benefitsTitle).toHaveClass('text-elegant', 'text-3xl', 'md:text-4xl', 'font-semibold', 'text-primary-800');
    });
    ...
    test('should display benefit descriptions with trust-building language', async () => {
      ...
      expect(screen.getByText(/květiny objednáváme vždy čerstvé až na základě vaší objednávky/i)).toBeInTheDocument();
      expect(screen.getByText(/rozumíme naléhavosti a zajistíme včasné dodání bez stresu/i)).toBeInTheDocument();
      expect(screen.getByText(/každý věnec je jedinečné dílo vytvořené s láskou a úctou/i)).toBeInTheDocument();
      expect(screen.getByText(/pomůžeme vám vytvořit osobní a významné rozloučení/i)).toBeInTheDocument();
    });
  ...
  describe('Philosophy Section Trust Elements', () => {
    test('should display philosophical quote with elegant styling', async () => {
      ...
      const quote = screen.getByText('Život je křehký jako motýlí prach, ale krása a láska zůstávají navždy');
      expect(quote).toBeInTheDocument();
      expect(quote).toHaveClass('text-2xl', 'md:text-3xl', 'text-primary-700', 'font-elegant', 'italic');
    });
    ...
    test('should display philosophical explanation', async () => {
      ...
      const philosophyText = screen.getByText(/uvědomujeme si, že život je křehký jako motýlí prach/i);
  ...
  describe('Features Section Value Propositions', () => {
    test('should display handcrafted feature with icon', async () => {
      ...
      expect(screen.getByText(/každý věnec je pečlivě vytvořen našimi zkušenými floristy/i)).toBeInTheDocument();
    });
  ...
  describe('User Journey Validation', () => {
    ...
    test('should have proper responsive design classes', async () => {
      ...
      const ctaContainer = screen.getByRole('link', { name: /vybrat věnec pro rozloučení/i }).closest('.flex');
      expect(ctaContainer).toHaveClass('flex', 'flex-col', 'sm:flex-row', 'gap-6', 'justify-center');
    });
