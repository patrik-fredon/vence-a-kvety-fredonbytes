...
jest.mock('next-intl/server', () => ({
  getTranslations: jest.fn(() => (key: string) => {
    const translations: Record<string, string> = {
      ...
      'mission': 'Pomáháme rodinám vyjádřit lásku a úctu k jejich blízkým prostřednictvím krásných květinových aranžmá v nejtěžších chvílích života.',
      'story': 'Jsme malá rodinná květinová dílnička s dlouholetými zkušenostmi, která svou práci dělá s citem a důrazem na každý detail. Květinám se věnujeme řadu let a věříme, že sféra smutečních květin si zaslouží více pozornosti a krásy. Do každé vaší objednávky dáváme kousek svého srdce a profesionální přístup, aby tato smutná událost ve vašich životech byla alespoň trochu příjemnější.',
      'values': 'Každý květ vybíráme s odbornou pečlivostí a kvalitou, aby výsledek naší práce splňoval všechna vaša očekávání. Květiny kupujeme vždy čerstvé, pečlivě je čistíme a ošetřujeme s profesionální péčí, aby památku vašeho milovaného uctívaly na pietním místě co nejdéle.',
      'commitment': 'Naším závazkem je poskytovat nejen krásné květinové aranžmá, ale také porozumění, empatie a podporu v těžkých chvílích. Věříme, že kvalitní ruční práce a osobní přístup mohou přinést útěchu a důstojnost do procesu rozloučení.'
    };
...
describe('About Page', () => {
  ...
  describe('Page Structure', () => {
    it('should render the main heading', async () => {
      ...
      expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('O naší květinové dílničce');
    });
  ...
  describe('Content Sections', () => {
    it('should display company story section', async () => {
      ...
      expect(screen.getByText(/Jsme malá rodinná květinová dílnička s dlouholetými zkušenostmi/)).toBeInTheDocument();
    });
    ...
    it('should display commitment section', async () => {
      ...
      expect(screen.getByText(/Naším závazkem je poskytovat nejen krásné květinové aranžmá/)).toBeInTheDocument();
    });
    ...
    it('should display company information section', async () => {
      ...
      expect(screen.getByText('Rodinná květinová dílnička s tradicí a důrazem na kvalitu')).toBeInTheDocument();
    });
  ...
  describe('Responsive Design', () => {
    ...
    it('should have responsive button layout', async () => {
      ...
      const buttonContainer = screen.getByRole('link', { name: 'Prohlédnout naše věnce' }).parentElement;
