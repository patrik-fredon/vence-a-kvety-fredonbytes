...
jest.mock("next-intl/server", () => ({
  getTranslations: jest.fn(() => (key: string) => {
    const translations: Record<string, string> = {
      ...
      "items.0.answer":
        "Pokud si přejete věnec doručit den nebo dva před obřadem, doporučujeme jej nechat vodorovně uložený v původní krabici. Zajistíte tak, že voda z korpusu věnce neodteče. Žádná další péče není nutná – věnec je dostatečně nasycen vodou a květiny jsou ošetřeny pro co nejdelší výdrž.",
      "items.1.question": "Jak dlouho zůstanou květiny v pohřebním věnci čerstvé?",
      "items.1.answer":
        "Výdrž květin závisí na povětrnostních podmínkách. Obecně platí, že čím kratší stonek, tím delší výdrž. Smuteční věnce jsou určeny k vystavení na pietním místě. V létě při vysokých teplotách vydrží věnec krásný po dobu jednoho týdne, v chladnějším období může vydržet až dva týdny.",
      "items.2.question": "Co dělat s věncem po uvadnutí květin?",
      "items.2.answer":
        "Doporučujeme ekologickou recyklaci. Květiny z věnce vyhoďte do sběrné popelnice určené pro bio odpad a korpus věnce do sběru plastů. Společně tak šetříme přírodu a dbáme na odpovědné nakládání s materiály.",
    };
...
describe("FAQ Page", () => {
  ...
  describe("Page Structure", () => {
    ...
    it("should render the page description", async () => {
      ...
      expect(
        screen.getByText(/Odpovědi na nejčastější otázky o našich pohřebních věncích/)
      ).toBeInTheDocument();
    });
  ...
  describe("Content Integration", () => {
    ...
    it("should display all FAQ questions", async () => {
      ...
      expect(
        screen.getByText("Jak správně skladovat a pečovat o pohřební věnec?")
      ).toBeInTheDocument();
      expect(
        screen.getByText("Jak dlouho zůstanou květiny v pohřebním věnci čerstvé?")
      ).toBeInTheDocument();
    ...
    it("should display all FAQ answers", async () => {
      ...
      expect(
        screen.getByText(/Pokud si přejete věnec doručit den nebo dva před obřadem/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Výdrž květin závisí na povětrnostních podmínkách/)
      ).toBeInTheDocument();
