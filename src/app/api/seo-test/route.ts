  ...
  try {
    ...
    if (testType === 'all' || testType === 'metadata') {
      ...
      const homepageMetadata = {
        ...
        description: 'Prémiové pohřební věnce a květinové aranžmá. Ruční výroba, pečlivý výběr květin, rychlé dodání.',
    ...
    const totalScore = allResults.reduce((sum: number, result: any) => sum + (result.score || 0), 0);
    const averageScore = allResults.length > 0 ? Math.round(totalScore / allResults.length) : 0;
    const totalErrors = allResults.reduce((sum: number, result: any) => sum + (result.errors?.length || 0), 0);
    const totalWarnings = allResults.reduce((sum: number, result: any) => sum + (result.warnings?.length || 0), 0);
    ...
    results.summary = {
      ...
      recommendations: totalErrors > 0 || averageScore < 80 ? [
