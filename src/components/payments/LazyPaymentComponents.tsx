...
export const LazyStripePaymentForm = dynamic(
  () => import("./StripePaymentForm").then((mod) => ({ default: mod.StripePaymentForm })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-6 border border-stone-200 rounded-lg">
        <LoadingSpinner size="md" />
        <span className="ml-2 text-stone-600">Loading Stripe Payment...</span>
      </div>
    ),
  ...
  () => import("@stripe/react-stripe-js").then((mod) => ({ default: mod.Elements })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-6">
        <LoadingSpinner size="md" />
        <span className="ml-2 text-stone-600">Loading Payment Provider...</span>
      </div>
    ),
    ssr: false,
  }
