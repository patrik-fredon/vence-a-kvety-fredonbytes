      ...
      <Card className="p-6">
        ...
        {coreWebVitals.getOptimizations().length > 0 && (
          <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg mb-6">
            <h3 className="text-lg font-semibold text-amber-800 mb-2">Optimization Recommendations</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              onClick={handleSimulateSlowTask}
              variant="outline"
              disabled={isLoading}
            >
          ...
          {Array.from({ length: imageCount }, (_, index) => (
            <div key={index} style={clsPreventionStyles} className="bg-stone-100 rounded-lg overflow-hidden">
              <OptimizedImage
                ...
                priority={index === 0}
                onLoad={() => handleImageLoad(`demo-image-${index}`, performance.now(), index === 0)}
              />
            </div>
          ))}
        </div>
      ...
      {layoutShiftDemo && (
        <Card className="p-6 bg-red-50 border-red-200">
          ...
          <p className="text-red-700">
            This content appeared without reserved space, causing a layout shift.
            In production, this would negatively impact CLS scores.
          </p>
        </Card>
      )}
