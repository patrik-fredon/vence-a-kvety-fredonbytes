  ...
  describe('Input Component', () => {
    ...
    it('should be accessible with labels', async () => {
      const { container } = render(
        <Input label="Test Label" placeholder="Test input" />
      );
      const results = await axe(container);
      expect(results).toHaveNoViolations();
    });
...
describe('Design System Integration', () => {
  it('should work together harmoniously', async () => {
    const { container } = render(
      <div className="p-6 space-y-6">
        ...
        <Card variant="memorial">
          <CardHeader>
            <Heading as="h2" size="lg">Featured Arrangement</Heading>
          </CardHeader>
          <CardContent>
    ...
    expect(screen.getByText('Professional and respectful floral arrangements for memorial services.')).toBeInTheDocument();
